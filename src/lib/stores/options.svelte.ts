			...
			if (!options) {
				...
				if (!pulled) {
					...
					options = {
						...
						taxRate: 6.5,
						invoiceDueDays: 30,
						notifyCrewAssignmentEmail: true,
						notifyCrewAssignmentPush: true,
						autoEmailInvoiceOnGenerate: false,
						areasOfTown: [],
	...
	async _doPullFromPB() {
		try {
			...
			const serverOptions = {
				...
				notifyCrewAssignmentEmail: record.notifyCrewAssignmentEmail !== false,
				notifyCrewAssignmentPush: record.notifyCrewAssignmentPush !== false,
				autoEmailInvoiceOnGenerate: record.autoEmailInvoiceOnGenerate === true,
	...
	async syncToPB(updatedData: any) {
		...
		try {
			...
			const pbPayload = {
				...
				notifyCrewAssignmentEmail: cleanData.notifyCrewAssignmentEmail !== false,
				notifyCrewAssignmentPush: cleanData.notifyCrewAssignmentPush !== false,
				autoEmailInvoiceOnGenerate: cleanData.autoEmailInvoiceOnGenerate === true,
