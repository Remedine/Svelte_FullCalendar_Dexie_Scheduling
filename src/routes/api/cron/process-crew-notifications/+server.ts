...
export async function POST({ request }: { request: Request }) {
	try {
		...
		if (!options) {
			// Return 200 so Railway cron does not treat a transient PB blip as a crash.
			return json({ skipped: true, reason: 'Options not found' });
		}
		...
		console.error('[cron] process-crew-notifications failed', err);
		return json({
			success: false,
